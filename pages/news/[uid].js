  ...
  return (
    <Layout navigation={navigation}>
      ...
      <div className='container news-page'>
          <img className="main" src={page.data.image.full ? page.data.image.full.url : page.data.image.url }/>
          {page.data.date &&
            <div className="date">
              {new Date(page.data.date).toLocaleDateString("en-US", { year: 'numeric' }) } {new Date(page.data.date).toLocaleDateString("en-US", { month: 'long' }) } {new Date(page.data.date).toLocaleDateString("en-US", { day: 'numeric' }) }
            </div>
          } 
...
export async function getStaticPaths() {
  ...
  const pages = await client.getAllByType("news_item");
